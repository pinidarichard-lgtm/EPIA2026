...
export default function ProfNotes() {
  ...
  async function loadConfigs() {
    const [{ data: cfg }, { data: e }] = await Promise.all([
      supabaseAdmin.from("ib_matieres_config").select("*").eq("annee_scolaire", selAnnee).eq("annee_pd", selPd).eq("actif", true).order("groupe_matiere").order("matiere"),
      supabaseAdmin.from("eleves").select("*").eq("annee_pd", selPd).eq("annee_scolaire", selAnnee).eq("actif", true).order("nom"),
    ]);
    setConfigs(cfg || []);
    setEleves(e || []);
    setSelConfig(null);
          ...
          <div>
            ...
            {configs.length === 0
              ? <div style={{ fontSize:12,color:"#aaa",textAlign:"center",padding:"1rem" }}>
                  Aucune matière configurée pour {selPd} — {selAnnee}.
                </div>
