...
const supabase = createClient(
  ...
  import.meta.env.VITE_SUPABASE_ANON_KEY  // ← clé ANON (publique) pour la connexion
);
...
export default function AdminLogin() {
  ...
  async function handleLogin(e) {
    ...
    const { data: profile, error: profileError } = await supabase
      .from("profiles")           // ← adaptez si votre table s'appelle autrement
      .select("role")
      .eq("id", data.user.id)
      .single();
    ...
    // 3. Accès accordé → redirection vers le panel admin
    navigate("/admin");
  }
  ...
  const s = {
    ...
    card: {
      ...
      padding: "2rem", width: 360, maxWidth: "92vw",
    },
    logo: {
      width: 48, height: 48, borderRadius: 12, background: "#1B3A6B",
      display: "flex", alignItems: "center", justifyContent: "center",
      color: "#fff", fontWeight: 700, fontSize: 18, marginBottom: "1.25rem",
    },
    ...
    btn: {
      width: "100%", padding: "10px", background: "#1B3A6B", color: "#fff",
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <p style={s.subtitle}>EPIA Lomé — Programme du Diplôme</p>
