  ...
  return (
    <div style={{ paddingTop: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        <input type="text" placeholder="Rechercher par matière ou enseignant…" value={search}
      ...
      {!loading && !error && filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '4rem 2rem', background: '#fff', borderRadius: 12, border: '1px solid #e8e6e0' }}>
          ...
          <p style={{ color: '#888', fontSize: 16 }}>{plans.length === 0 ? "Aucun plan d'unité pour l'instant." : 'Aucun résultat.'}</p>
        </div>
      )}
    ...
    <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e8e6e0', padding: '1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
      <div style={{ flex: 1, minWidth: 200 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
          <span style={{ fontWeight: 600, fontSize: 16, color: '#1a1a1a' }}>{plan.matiere || 'Matière non définie'}</span>
          ...
          {plan.annee_pd && <span style={{ background: '#f0f0f0', color: '#555', fontSize: 12, padding: '2px 8px', borderRadius: 20 }}>{plan.annee_pd}ᵉ année PD</span>}
          <span style={{
