...
export default function PlansListPage() {
  ...
  async function fetchPlans() {
    try {
      ...
      const { data, error } = await supabase
        .from('unit_plans')
        .select('id, matiere, enseignants, annee_scolaire, groupe_matieres, niveau, annee_pd, semestre, statut, created_at')
  ...
  const filtered = plans.filter(p => {
    ...
    const matchNiveau = !filterNiveau || p.niveau === filterNiveau
    ...
    <div style={{ paddingTop: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        <input
          ...
          style={{
            ...
            border: '1px solid #ddd', borderRadius: 8, fontSize: 14, background: '#fff'
          }}
        />
      ...
      {error && (
        <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 8, padding: '1rem', color: '#991b1b', marginBottom: '1rem' }}>
          Erreur : {error}. Vérifiez vos variables d'environnement Supabase.
        </div>
      )}
...
function PlanCard({ plan, onDelete }) {
  const niveauColor = plan.niveau === 'NS' ? '#1a6b4a' : '#1a3a5c'
