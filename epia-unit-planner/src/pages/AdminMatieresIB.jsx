...
export default function AdminMatieresIB() {
  ...
  useEffect(() => { loadInit(); }, []);
  useEffect(() => { if (selAnnee) loadConfigs(); }, [selAnnee, selPd]);
  ...
  async function loadConfigs() {
    const { data } = await supabaseAdmin.from("ib_matieres_config")
      .select("*").eq("annee_scolaire", selAnnee).eq("actif", true).order("groupe_matiere").order("matiere").order("niveau");
    setConfigs(data || []);
  }

  function openAdd() {
    setEditId(null);
    setFGroupe(""); setFMatiere(""); setFNiveau("NM"); setFAnnee(selAnnee); setFPd(selPd);
    setFEpreuves([
      { nom:"Épreuve 1", bareme:20, pourcentage:30 },
      { nom:"Épreuve 2", bareme:20, pourcentage:40 },
      { nom:"Évaluation Interne", bareme:20, pourcentage:30 },
  ...
  async function duplicate(c) {
    ...
    await supabaseAdmin.from("ib_matieres_config").insert({
      matiere: c.matiere, groupe_matiere: c.groupe_matiere,
      niveau: newNiv, annee_scolaire: c.annee_scolaire, annee_pd: c.annee_pd,
      epreuves: c.epreuves, frontieres: c.frontieres, actif: true,
    });
    await loadConfigs();
    showBanner(`✓ Copié en ${newNiv}.`, "success");
  }
