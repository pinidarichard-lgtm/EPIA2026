...
export default function AdminMatieresIB() {
  ...
  // Sync modal default pd with active tab
  useEffect(() => { setFPd(selPd); }, [selPd]);
  ...
  async function duplicate(c) {
    ...
    const { data: existing } = await supabaseAdmin.from("ib_matieres_config")
      .select("id").eq("matiere", c.matiere).eq("niveau", newNiv)
      .eq("annee_scolaire", c.annee_scolaire).eq("annee_pd", c.annee_pd).single();
    if (existing) {
      showBanner(`Une config ${newNiv} existe déjà pour cette matière.`, "error");
      return;
    }
    ...
    if (error) { showBanner("Erreur duplication : " + error.message, "error"); return; }
    await loadConfigs();
